...
class MethodParam:
    ...
    def to_dict(self, pool: Optional["ConstantPool"] = None) -> MethodParamDict:
        ...
        return {"kind": self.kind, "name": self.name, "default_value": dv_dict}
...
class MethodBody:
    ...
    def to_string(
        self, pool: Optional["ConstantPool"] = None, show_offsets: bool = True
    ) -> str:
        ...
        reader = ABCReader(b"")
        return reader.serialize_instructions_to_string(
            self.instructions, pool, show_offsets
        )
    ...
    def __repr__(self) -> str:
        return (
            f"MethodBody(method={self.method}, stack={self.max_stack}, "
    ...
    def __str__(self) -> str:
        ...
        lines = []
        lines.append(
            f"MethodBody(method={self.method}, stack={self.max_stack}, locals={self.num_locals})"
        )
        lines.append(
            f"  init_scope_depth={self.init_scope_depth}, max_scope_depth={self.max_scope_depth}"
        )
        lines.append(
            f"  code_length={len(self.code)}, exceptions={len(self.exceptions)}, traits={len(self.traits)}"
        )
        ...
        if self.instructions:
            ...
            for inst in self.instructions:
                ...
                operands_str = (
                    ", ".join(str(op) for op in inst.operands) if inst.operands else ""
                )
                lines.append(
                    f"    [{inst.offset:3d}] {inst.opcode.name:20} {operands_str}"
                )
