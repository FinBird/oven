...
from .debug import DebugCommentGenerator


_AVM2_CFG_ADAPTER = AVM2ControlFlowAdapter()

# Registry of pipeline stage names to pass classes
_PIPELINE_STAGE_REGISTRY: dict[str, type] = {
    "AstSemanticNormalizePass": AstSemanticNormalizePass,
    "MoveStaticInitsToFieldsPass": MoveStaticInitsToFieldsPass,
    "NamespaceCleanupPass": NamespaceCleanupPass,
    "ImportDiscoveryPass": ImportDiscoveryPass,
    "TextCleanupPass": TextCleanupPass,
    "AstConstructorCleanupPass": AstConstructorCleanupPass,
    # TODO: add remaining passes as they are implemented
}
...
def _method_to_nf(body: MethodBody) -> Node:
    ...
    try:
        result = pipeline.transform(body.instructions, body)
        ...
        ast, _, _ = ASTBuild({"tolerate_stack_underflow": True}).transform(
        ...
        if not fallback.children:
            fallback.children = [
                Node("stack_hole", ["decompile_fallback"], {"synthetic": 1})
            ]
...
class MethodContext:
    ...
    discovered_imports: set[str] = field(default_factory=set)
    ...
    @property
    def rest_register_index(self) -> int | None:
        if self.rest_param_name is None:
            return None
        return self.param_register_start + len(self.param_names)
...
def _normalize_int_format(int_format: str) -> str:
    ...
    if token in {"hex", "hexadecimal"}:
        return "hex"
    raise ValueError(
        f"unsupported integer format: {int_format!r} (expected 'dec' or 'hex')"
    )
...
def _visibility_from_namespace_kind(
    namespace_kind: str | None, *, for_class: bool = False
) -> str:
...
def _sanitize_identifier(raw: object, fallback: str, used: set[str]) -> str:
    ...
    # Preserve leading underscore (e.g. _data) while cleaning trailing ones.
    leading = len(token) - len(token.lstrip("_"))
    token = token.lstrip("_").rstrip("_")
    if leading and token:
        token = "_" + token
    if not token:
        token = fallback
    if token[0].isdigit():
...
def _append_slot_names_from_traits(
    slot_name_map: dict[int, str], traits: object
) -> None:
...
def _trait_slot_id(trait: object) -> int | None:
    data = getattr(trait, "data", None)
    if not isinstance(data, dict):
        return None
    slot_id = _index_value(data.get("slot_id"))
...
def _ordered_field_traits(traits: list[object]) -> list[object]:
    def _sort_key(item: tuple[int, object]) -> tuple[int, int, int, str]:
        raw_index, trait = item
        slot_id = _trait_slot_id(trait)
        trait_name = _short_multiname(getattr(trait, "name", ""))
        if slot_id is None:
            return (1, raw_index, raw_index, trait_name)
        return (0, slot_id, raw_index, trait_name)

    return [trait for _idx, trait in sorted(enumerate(traits), key=_sort_key)]


def _build_owner_slot_name_map(
    abc: ABCFile, owner_kind: str, owner_index: int | None
) -> dict[int, str]:
    ...
    if owner_kind == "instance":
        if owner_index < len(abc.instances):
            _append_slot_names_from_traits(
                slot_name_map, getattr(abc.instances[owner_index], "traits", None)
            )
        if owner_index < len(abc.classes):
            _append_slot_names_from_traits(
                slot_name_map, getattr(abc.classes[owner_index], "traits", None)
            )
    ...
    if owner_kind == "script" and owner_index < len(abc.scripts):
        _append_slot_names_from_traits(
            slot_name_map, getattr(abc.scripts[owner_index], "traits", None)
        )
        cache[cache_key] = slot_name_map
        return slot_name_map
    cache[cache_key] = slot_name_map
...
def _register_owner(
    ...
    if current is None or _OWNER_PRIORITY.get(owner_kind, 0) > _OWNER_PRIORITY.get(
        current.owner_kind, 0
    ):
    ...
    for idx, instance in enumerate(abc.instances):
        ...
        for trait in getattr(instance, "traits", []):
            ...
            if method_index is None:
                continue
            method_name = (
                _short_multiname(getattr(trait, "name", f"method_{method_index}"))
                or f"method_{method_index}"
            )
            _register_owner(
                owners,
                method_index,
...
def _build_param_names(
    method_info: MethodInfo, owner_kind: str
    ...
    for position, param in enumerate(method_info.params, start=1):
        ...
        base_name = (
            ...
            else f"param{position}"
        )
        names.append(_sanitize_identifier(base_name, f"param{position}", used))
    return tuple(names), has_param_names


def _build_rest_param_name(
    method_info: MethodInfo,
    owner_kind: str,
    param_names: tuple[str, ...],
) -> str | None:
    if not (method_info.flags & MethodFlags.NEED_REST):
        return None

    used = set(param_names)
    if owner_kind == "instance":
        used.add("this")
    return _sanitize_identifier("rest", "rest", used)
...
def _build_method_context(
    ...
    method_name = (
        owner.method_name if owner and owner.method_name else default_method_name
    )
    param_names, has_param_names = _build_param_names(method_info, owner_kind)
    rest_param_name = _build_rest_param_name(method_info, owner_kind, param_names)
...
def _split_qualified_name(
    ...
    class_name = _sanitize_identifier(
        _short_multiname(class_token) or fallback, fallback, set()
    )
...
def _collect_class_method_entries(
    abc: ABCFile,
    class_index: int,
) -> tuple[tuple[str, ...], str, list[_ClassMethodEntry], list[int]]:
    ...
    trait_indices: list[int] = list(range(len(getattr(instance, "traits", []))))
    if cls:
        start = len(getattr(instance, "traits", []))
        trait_indices.extend(range(start, start + len(getattr(cls, "traits", []))))
    ...
    for trait in getattr(instance, "traits", []):
        ...
        _append(
            ...
            namespace_kind=_namespace_kind_from_qualified_name(
                getattr(trait, "name", "")
            ),
        )
    ...
    return package_parts, class_name, entries, trait_indices
...
def _build_method_ir(
    ...
    *,
    ...
    namespace_cleanup: bool = False,
    enable_auto_imports: bool = False,
    enable_text_cleanup: bool = False,
    ...
    if style == "semantic":
        ...
        stages_to_run = pipeline_stages if pipeline_stages is not None else []
        # If no explicit stages, fall back to flag-based logic for backward compatibility
        if not stages_to_run:
            # Build default stages based on flags (mimicking old behavior)
            default_stages = []
            default_stages.append("AstSemanticNormalizePass")
            if (
                context is not None
                and context.method_name == "__static_init__"
                and class_traits is not None
                and abc is not None
            ):
                default_stages.append("MoveStaticInitsToFieldsPass")
            if (
                context is not None
                and context.owner_kind == "instance"
                and context.method_name == context.owner_name
            ):
                default_stages.append("AstConstructorCleanupPass")
            if namespace_cleanup:
                default_stages.append("NamespaceCleanupPass")
            if enable_auto_imports and abc is not None:
                default_stages.append("ImportDiscoveryPass")
            if enable_text_cleanup:
                default_stages.append("TextCleanupPass")
            stages_to_run = default_stages

        # Apply each stage in order
        for stage_name in stages_to_run:
            pass_cls = _PIPELINE_STAGE_REGISTRY.get(stage_name)
            if pass_cls is None:
                # Unknown stage; ignore
                continue
            # Instantiate pass with appropriate arguments
            if pass_cls is AstSemanticNormalizePass:
                slot_name_map = context.slot_name_map if context is not None else None
                global_slot_name_map = (
                    context.global_slot_name_map if context is not None else None
                )
            ...
            elif pass_cls is MoveStaticInitsToFieldsPass:
                if not (
                    context is not None
                    and context.method_name == "__static_init__"
                    and class_traits is not None
                    ...
                    continue
                nf = MoveStaticInitsToFieldsPass(
                    owner_name=_class_display_name(
                        context.owner_name or "Class",
                        context.owner_name or "Class",
                    ),
            ...
            elif pass_cls is AstConstructorCleanupPass:
                if not (
                    context is not None
                    and context.owner_kind == "instance"
                    and context.method_name == context.owner_name
            ...
            elif pass_cls is NamespaceCleanupPass:
                nf = NamespaceCleanupPass().transform(nf)
            elif pass_cls is ImportDiscoveryPass:
                if abc is None:
                    continue
                nf = ImportDiscoveryPass(
                    abc_obj=abc,
                    current_fqcn=current_fqcn,
                    method_context=context,
                    manual_import_mapping=manual_import_mapping,
                ).transform(nf)
            elif pass_cls is TextCleanupPass:
                nf = TextCleanupPass().transform(nf)
            else:
                # Generic instantiation (no arguments)
                nf = pass_cls().transform(nf)
    return nf, context
...
def _param_name_for_local(context: MethodContext | None, local_index: int) -> str:
    if context is not None:
        ...
        if context.rest_register_index == local_index and context.rest_param_name:
            return context.rest_param_name
...
def _try_fast_emit_method_text(
    ...
    *,
    ...
    # Fast-path templates only cover very small methods.
    # Avoid building opcode tuples for large bodies where this path never matches.
    ...
    if (
        len(opcodes) >= 5
        and opcodes[0:2] == ("GetLocal0", "PushScope")
        and opcodes[-2:] == ("ConstructSuper", "ReturnVoid")
    ):
        ...
        if get_local_indexes and get_local_indexes[0] == 0:
            arg_names = [
                _param_name_for_local(context, idx) for idx in get_local_indexes[1:]
            ]
            if arg_names:
    ...
    if opcodes == ("GetLocal0", "PushScope", "PushByte", "ReturnValue"):
        if len(instructions[2].operands) == 1 and isinstance(
            instructions[2].operands[0], int
        ):
...
def _resolve_default_value_expr(
    abc: ABCFile,
    ...
    *,
    ...
    if kind in {
        ...
        if kind == ConstantKind.INT:
            return _format_fast_int_literal(
                int(abc.constant_pool.resolve_index(index, "int")),
                int_format,
            )
        if kind == ConstantKind.UINT:
            return _format_fast_int_literal(
                int(abc.constant_pool.resolve_index(index, "uint")),
                int_format,
            )
        ...
        text_str = str(text)
        # If the string looks like a class name (contains ::), treat as class reference, return short name (unquoted)
        if "::" in text_str:
            # Extract short name
            short = text_str.split("::")[-1]
            # Optionally add import for fully qualified class name? Need context, skip for now.
            return short
...
def _trait_default_initializer_expr(
    abc: ABCFile,
    trait: object,
    *,
    int_format: str = "dec",
    ...
    if isinstance(value, DefaultValue):
        return _resolve_default_value_expr(abc, value, int_format=int_format)
    if isinstance(value, bool):
        return "true" if value else "false"
    if isinstance(value, str):
        return json.dumps(value, ensure_ascii=False)
    if isinstance(value, int):
        return _format_fast_int_literal(value, int_format)
    if isinstance(value, float):
        return f"0x{int(value):X}"

    # Handle class reference constants (like P_ReturnCode for USERDETAILINFO)
    if isinstance(value, int) and value > 0:
        # Try to resolve as a multiname index
        try:
            resolved = abc.constant_pool.resolve_index(value, "multiname")
            if resolved:
                short_name = _short_multiname(str(resolved))
                if short_name and short_name != "*":
                    return short_name
        except (AttributeError, IndexError, TypeError):
            pass

    # Handle references to other classes/interfaces
    try:
        if hasattr(value, "resolve") and callable(getattr(value, "resolve", None)):
            resolved = value.resolve(abc.constant_pool, "multiname")
            if resolved:
                short_name = _short_multiname(str(resolved))
                if short_name and short_name != "*":
                    return short_name
    except (AttributeError, TypeError):
        # value might be int or other non-resolvable type
        pass
...
def _build_class_member_lines(
    ...
    *,
    ...
    static_field_initializers: dict[str, str] | None = None,
    int_format: str = "dec",
) -> tuple[list[str], set[str]]:
    lines: list[str] = []
    declared_names: set[str] = set()
    used_identifiers: set[str] = set()
    static_field_initializers = static_field_initializers or {}
    ...
    def _append_trait_member(trait: object, *, is_static: bool) -> None:
        ...
        if is_static and name in static_field_initializers:
            init_expr = static_field_initializers[name]
        elif not is_static and init_expr is None:
    ...
    # Match JPEXS class layout: static slot/const traits first, then instance ones.
    # Within each section, prefer slot_id order for deterministic declarations.
...
def _build_class_signature(
    ...
    if instance is not None:
        ...
        # Determine if this is an interface (prefer parameter, fallback to instance attribute)
        if not is_interface:
        ...
        if super_name:
            if hasattr(super_name, "resolve"):
                raw = super_name.resolve(abc_obj.constant_pool, "multiname")
            else:
                raw = super_name
            super_token = _short_multiname(raw).strip()
        ...
        if isinstance(interfaces, list):
            for iface in interfaces:
                if hasattr(iface, "resolve"):
                    raw = iface.resolve(abc_obj.constant_pool, "multiname")
                else:
                    raw = iface
                token = _short_multiname(raw).strip()
                # Handle namespace set format like "[NsSet]::INetSystem"
                if token.startswith("[NsSet]::"):
                    token = token[10:]  # Remove "[NsSet]::" prefix
    ...
    # 构建类签名
    class_or_interface = "interface" if is_interface else "class"
    signature_parts = [visibility, class_or_interface, class_name]
...
def _field_initializer_name(value: object) -> str | None:
    ...
    if isinstance(token, Node):
        if token.type == "string" and token.children:
            ...
            token.type in {"get_lex", "find_property", "find_property_strict"}
            and token.children
        ):
            token = token.children[0]
        else:
...
def _extract_method_field_initializers(
    ...
    *,
    ...
    emitter = AS3Emitter(
        ...
        debug_include_offset=debug_include_offset,
        debug_include_opcode=debug_include_opcode,
        debug_include_operands=debug_include_operands,
    )
    ...
    for child in nf.children:
        ...
        for entry in child.children:
            if (
                not isinstance(entry, Node)
                or entry.type != "field_initializer"
...
def _is_effectively_empty_method_nf(nf: object) -> bool:
    if not isinstance(nf, Node) or nf.type != "begin":
        return False

    ignorable = {"nop", "label", "push_scope", "pop_scope", "return_void"}
    for child in nf.children:
        if isinstance(child, Node) and child.type == "begin":
            if not _is_effectively_empty_method_nf(child):
                return False
            continue
        if isinstance(child, Node) and child.type in ignorable:
            continue
        return False
    return True
...
def _wrap_package_block(package_parts: tuple[str, ...], source: str) -> str:
    return _wrap_package_block_with_imports(package_parts, source, ())


def _wrap_package_block_with_imports(
    package_parts: tuple[str, ...],
    source: str,
    imports: tuple[str, ...] | list[str] | set[str],
) -> str:
    ...
    import_lines = [
        f"import {item};"
        for item in sorted({str(item).strip() for item in imports if str(item).strip()})
    ]
    merged = source
    if import_lines:
        merged = "\n".join(import_lines) + "\n\n" + source
    return f"package {package_name} {{\n{_indent_block(merged)}\n}}"
...
def _render_layout_method_signature(
    *,
    ...
    visibility = _visibility_from_namespace_kind(entry.namespace_kind)
    ...
    for idx, param in enumerate(method_info.params, start=1):
        ...
        if abc is not None and isinstance(default_value, DefaultValue):
            default_expr = _resolve_default_value_expr(
                abc, default_value, int_format=int_format
            )
        if default_expr is not None:
            param_spec = f"{param_spec} = {default_expr}"
        param_specs.append(param_spec)
    if method_info.flags & MethodFlags.NEED_REST:
        rest_name = (
            context.rest_param_name
            if context is not None and context.rest_param_name
            else "rest"
        )
        param_specs.append(f"... {rest_name}")
    ...
    if entry.trait_kind == TraitKind.GETTER:
        return (
            f"{visibility}{static_kw} function get {entry.method_name}():{return_type}"
        ).strip()
    if entry.trait_kind == TraitKind.SETTER:
        setter_param = param_specs[0] if param_specs else "value:*"
        return (
            f"{visibility}{static_kw} function set {entry.method_name}({setter_param}):void"
        ).strip()
    return (
        f"{visibility}{static_kw} function {entry.method_name}({params_text}):{return_type}"
    ).strip()
...
class AS3Emitter:
    ...
    _local_name_map: dict[int, str] = field(
        default_factory=dict, init=False, repr=False
    )
    _declared_locals: list[LocalDeclaration] = field(
        default_factory=list, init=False, repr=False
    )
    _declared_local_indexes: set[int] = field(
        default_factory=set, init=False, repr=False
    )
    _expr_handler_cache: dict[str, object] = field(
        default_factory=dict, init=False, repr=False
    )
    _declared_local_types: dict[int, str] = field(
        default_factory=dict, init=False, repr=False
    )
    _inline_candidate_locals: set[int] = field(
        default_factory=set, init=False, repr=False
    )
    _inline_declared_locals: set[int] = field(
        default_factory=set, init=False, repr=False
    )
    _activation_slot_alias_map: dict[str, str] = field(
        default_factory=dict, init=False, repr=False
    )
    _activation_alias_decls: list[tuple[str, str]] = field(
        default_factory=list, init=False, repr=False
    )
    _new_function_lift_cache: dict[int, tuple[str | None, str | None]] = field(
        default_factory=dict, init=False, repr=False
    )
    ...
    def emit(self, node: Node) -> str:
        ...
        self._activation_slot_alias_map = {}
        self._activation_alias_decls = []

        lines: list[str] = []
        if self._is_semantic_style() and node.type == "begin":
            self._declared_locals = self._build_local_declarations(node)
            self._declared_local_types = {
                decl.index: decl.type_name for decl in self._declared_locals
            }
            if self.inline_vars:
                ...
                hoisted = [
                    ...
                    if decl.index not in self._inline_candidate_locals
                ]
                self._declared_local_indexes = {decl.index for decl in hoisted}
                for decl in hoisted:
                    lines.append(f"{self._pad(0)}var {decl.name}:{decl.type_name};")
            else:
                self._declared_local_indexes = {
                    decl.index for decl in self._declared_locals
                }
                for decl in self._declared_locals:
                    lines.append(f"{self._pad(0)}var {decl.name}:{decl.type_name};")
            self._prepare_activation_slot_aliases(node)
            if self._activation_alias_decls:
                known_names = set(self._local_name_map.values())
                known_names.update(decl.name for decl in self._declared_locals)
                for alias, type_name in self._activation_alias_decls:
                    if alias in known_names:
                        continue
                    lines.append(f"{self._pad(0)}var {alias}:{type_name};")
    ...
    def _build_local_name_map(self) -> dict[int, str]:
        ...
        if context.rest_register_index is not None and context.rest_param_name:
            mapping[context.rest_register_index] = context.rest_param_name
        return mapping

    @staticmethod
    def _unwrap_convert_wrappers(value: object) -> object:
        current = value
        while (
    ...
    def _is_activation_scope_subject(self, value: object) -> bool:
        current = self._unwrap_convert_wrappers(value)
        return isinstance(current, Node) and current.type == "get_scope_object"

    def _alias_activation_slot_name(self, name: str, subject_node: object) -> str:
        if not self._activation_slot_alias_map:
            return name
        if not self._is_activation_scope_subject(subject_node):
            return name
        return self._activation_slot_alias_map.get(name, name)

    def _prepare_activation_slot_aliases(self, root: Node) -> None:
        self._activation_slot_alias_map = {}
        self._activation_alias_decls = []
        if root.type != "begin":
            return

        reserved_names = set(self._local_name_map.values())
        reserved_names.update(decl.name for decl in self._declared_locals)

        for stmt in root.children:
            if (
                not isinstance(stmt, Node)
                or stmt.type != "set_property"
                or len(stmt.children) < 3
            ):
                continue

            subject_node = stmt.children[0]
            if not self._is_activation_scope_subject(subject_node):
                continue

            slot_name = self._property_name(stmt.children[1])
            if not slot_name.startswith("var_slot"):
                continue

            alias_name, alias_type = self._infer_activation_slot_alias(
                slot_name, stmt.children[2]
            )
            if alias_name is None:
                continue

            self._activation_slot_alias_map[slot_name] = alias_name
            if alias_name in reserved_names:
                continue
            if any(
                existing == alias_name for existing, _ in self._activation_alias_decls
            ):
                continue
            self._activation_alias_decls.append((alias_name, alias_type))
            reserved_names.add(alias_name)

    def _infer_activation_slot_alias(
        self, slot_name: str, value: object
    ) -> tuple[str | None, str]:
        current = self._unwrap_convert_wrappers(value)
        if isinstance(current, Node) and current.type == "null":
            existing = self._activation_slot_alias_map.get(slot_name)
            if existing is not None:
                type_name = "Function" if existing.startswith("gotoScene") else "*"
                return existing, type_name
            return None, "*"

        if (
            isinstance(current, Node)
            and current.type == "get_local"
            and current.children
        ):
            local_index = _index_value(current.children[0])
            if local_index is None:
                return None, "*"
            alias_name = self._var_name(local_index)
            context = self.method_context
            if (
                context is not None
                and context.method_name.endswith("ErrorHandler")
                and local_index == context.param_register_start
            ):
                alias_name = "back"
            if not _FIELD_IDENTIFIER_RE.match(alias_name):
                return None, "*"
            alias_type = "Object" if alias_name == "back" else "*"
            return alias_name, alias_type

        if (
            isinstance(current, Node)
            and current.type == "new_function"
            and current.children
        ):
            return None, "*"
    ...
    def _is_param_register(self, index: int) -> bool:
        ...
        return start <= index < end or context.rest_register_index == index
    ...
    def _build_local_declarations(self, root: Node) -> list[LocalDeclaration]:
        if self._exceeds_type_inference_budget(root):
            ...
            _, inferred_types = self._analyze_locals(
                root, node_budget=self._TYPE_INFERENCE_NODE_BUDGET
            )
        else:
    ...
    def _plan_inline_var_indexes(self, root: Node) -> set[int]:
        ...
        events: dict[int, dict[str, list[tuple[int, int, str, tuple[int, ...]]]]] = {
            idx: {"reads": [], "writes": []} for idx in tracked
        }
        ...
        def _is_descendant_scope(
            scope_path: tuple[int, ...], ancestor_path: tuple[int, ...]
        ) -> bool:
            return (
        ...
        def _record_read(
            local_index: int, depth: int, scope_path: tuple[int, ...]
        ) -> int:
        ...
        def _record_write(
            local_index: int, depth: int, kind: str, scope_path: tuple[int, ...]
        ) -> int:
            bucket = events.get(local_index)
            if bucket is None:
                return -1
        ...
        def _scan_expr(value: object, depth: int, scope_path: tuple[int, ...]) -> None:
            ...
            if value.type == "get_local" and value.children:
                ...
                value_reg = (
                    _index_value(value.children[0]) if len(value.children) > 0 else None
                )
                if value_reg is not None:
                    _record_write(value_reg, depth + 1, "other", loop_scope)
                object_reg = (
                    _index_value(value.children[2]) if len(value.children) > 2 else None
                )
        ...
        def _scan_stmt(value: object, depth: int, scope_path: tuple[int, ...]) -> None:
            ...
            if value.type == "if":
                ...
                if len(value.children) > 1:
                    _scan_stmt(
                        value.children[1], depth + 1, _next_scope_path(scope_path)
                    )
                if len(value.children) > 2:
                    _scan_stmt(
                        value.children[2], depth + 1, _next_scope_path(scope_path)
                    )
                return
        ...
        for local_index, bucket in events.items():
            ...
            first_write_seq, first_write_depth, first_write_kind, first_write_scope = (
                min(
                    writes,
            ...
            reads = bucket["reads"]
            if any(
                read_seq < first_write_seq for read_seq, _depth, _kind, _scope in reads
            ):
                continue
    ...
    def _analyze_locals(
        self, root: Node, node_budget: int | None = None
        ...
        for node in node_iter:
            ...
            if node.type in self._LOCAL_OPS and node.children:
                ...
                value_index = (
                    _index_value(node.children[0]) if len(node.children) > 0 else None
                )
                object_index = (
                    _index_value(node.children[2]) if len(node.children) > 2 else None
                )
                if value_index is not None:
                    local_indexes.add(value_index)
                    hinted = (
                        self._normalize_type_hint(node.children[1])
        ...
        if copy_edges:
            ...
            while changed:
                ...
                for target, source, edge_seq in copy_edges:
                    ...
                    if (
                        source_first_write_seq is None
                        or source_first_write_seq > edge_seq
                    ):
        ...
        for index, types in candidates.items():
            ...
            inferred_types[index] = (
                "*" if len(concrete_types) != 1 else next(iter(concrete_types))
            )
    ...
    def _normalize_type_hint(self, hint: object) -> str | None:
        if isinstance(hint, Node):
            ...
            if (
                hint.type in {"get_lex", "find_property", "find_property_strict"}
                and hint.children
            ):
                return self._normalize_type_hint(hint.children[0])
            return None
    ...
    def _infer_expr_type(
        self, value: object, local_type_cache: dict[int, str]
    ) -> str | None:
        if isinstance(value, bool):
            return "Boolean"
        if isinstance(value, int):
        ...
        if value.type in {"new_object", "new_activation", "new_catch"}:
            return "Object"
        if (
            value.type in {"get_lex", "find_property", "find_property_strict"}
            and value.children
        ):
        ...
        if (
            value.type in {"call_property", "call_property_lex", "call_super"}
        ...
        if value.type == "construct" and value.children:
            return (
                self._infer_expr_type(value.children[0], local_type_cache) or "Object"
            )
        ...
        if (
            value.type in {"ternary", "ternary_if", "ternary_if_boolean"}
    ...
    def _loop_binding(self, index: object) -> str:
        ...
        if (
            ...
            and local_index in self._declared_local_indexes
        ):
            return name
        return f"var {name}"
    ...
    def _emit_block(self, node: Node, level: int, inline: bool) -> list[str]:
        ...
        for i, child in enumerate(node.children):
            if isinstance(child, Node):
                # Skip return_void in method bodies (AS3 allows implicit return)
                if child.type == "return_void" and not inline:
                    continue
                ...
                lines.append(
                    f"{self._pad(level)}/* unsupported literal stmt: {child!r} */"
                )
    ...
    def _stmt(self, node: Node, level: int) -> str:
        ...
        debug_comment = ""
        if self.insert_debug_comments:
            instruction = node.metadata.get("instruction")
            if instruction is not None:
                metadata = {
                    "instruction": instruction,
                    "label": node.metadata.get("label", 0),
                }
                comment = DebugCommentGenerator.generate_from_metadata(
                    metadata,
                    include_offset=self.debug_include_offset,
                    include_opcode=self.debug_include_opcode,
                    include_operands=self.debug_include_operands,
                )
                if comment:
                    debug_comment = f"{self._pad(level)}{comment}"

        # Compute the statement content (original logic)
        stmt_content = self._compute_stmt_content(node, level)

        return debug_comment + stmt_content

    def _compute_stmt_content(self, node: Node, level: int) -> str:
        if node.type == "begin":
        ...
        if node.type == "jump_if":
            ...
            cond_value: object = (
                self._normalize_expr_node(cond_node)
            ...
            if target != "?":
                stmt_content = (
                    f"{self._pad(level)}if ({cond_expr}) {{ /* goto {target} */ }}"
                )
            else:
                stmt_content = f"{self._pad(level)}if ({cond_expr}) {{}}"
            return stmt_content
        ...
        if node.type == "while":
            ...
            for_init = (
                node.metadata.get("for_init")
                if isinstance(node.metadata, dict)
                else None
            )
            for_update = (
                node.metadata.get("for_update")
                if isinstance(node.metadata, dict)
                else None
            )
            if isinstance(for_init, Node) and isinstance(for_update, Node):
                ...
                lines = [
                    f"{self._pad(level)}for ({init_text}; {cond}; {update_text}) {{"
                ]
        ...
        if node.type == "for":
            ...
            body = (
                node.children[3]
                if len(node.children) > 3 and isinstance(node.children[3], Node)
                else Node("begin")
            )
            ...
            lines = [
                f"{self._pad(level)}for ({init_text}; {cond_text}; {update_text}) {{"
            ]
            lines.extend(self._emit_block(body, level + 1, inline=True))
            lines.append(f"{self._pad(level)}}}")
            return "\n".join(lines)
        ...
        if node.type == "for_in":
            ...
            lines = [
                f"{self._pad(level)}for ({self._loop_binding(value_reg)} in {self._var_name(object_reg)}) {{"
            ]
            lines.extend(self._emit_block(body, level + 1, inline=True))
            lines.append(f"{self._pad(level)}}}")
            return "\n".join(lines)
        ...
        if node.type == "for_each_in":
            ...
            lines = [
                f"{self._pad(level)}for each ({self._loop_binding(value_reg)} in {self._var_name(object_reg)}) {{"
            ]
            lines.extend(self._emit_block(body, level + 1, inline=True))
            lines.append(f"{self._pad(level)}}}")
            return "\n".join(lines)
        ...
        if node.type == "case":
            ...
            if (
                self._is_semantic_output_style()
                and isinstance(avm2_name, str)
                and avm2_name
            ):
        ...
        if node.type == "lookup_switch":
            ...
            expr = (
                self._expr(node.children[2]) if len(node.children) > 2 else "/* expr */"
            )
            return (
        ...
        if node.type == "catch":
            ...
            body = (
                node.children[2]
                if len(node.children) > 2 and isinstance(node.children[2], Node)
                else Node("begin")
            )
        ...
        if node.type == "delete":
            if len(node.children) >= 2:
                target = self._property_target(node.children)
                return f"{self._pad(level)}delete {target};"
            return f"{self._pad(level)}/* malformed delete */"
    ...
    def _emit_field_initializers(self, node: Node, level: int) -> str:
        ...
        for child in node.children:
            if (
                ...
                or child.type != "field_initializer"
                or len(child.children) < 2
            ):
                continue
    ...
    def _property_target(self, children: list[object]) -> str:
        ...
        prop_name = self._alias_activation_slot_name(prop_name, subject_node)
        if self._is_semantic_output_style():
            ...
            if self._is_redundant_member_access(subject, prop_name):
                return prop_name
            if (
                isinstance(subject_node, Node)
                and subject_node.type == "get_scope_object"
            ):
                return prop_name
    ...
    def _emit_if(self, node: Node, level: int, prefix: str = "if") -> str:
        ...
        cond_value: object = (
            self._normalize_expr_node(node.children[0])
            if node.children
            else Node("false")
        )
    ...
    def _normalize_expr_node(self, value: object) -> object:
        ...
        normalized_children = [
            self._normalize_expr_node(child) for child in value.children
        ]
        kind = value.type
        ...
        if (
            kind in {"ternary", "ternary_if", "ternary_if_boolean"}
            and len(normalized_children) >= 3
        ):
    ...
    def _expr_condition(self, value: object) -> str:
        normalized = self._normalize_expr_node(value)
        simplified = (
            self._strip_condition_wrappers(normalized)
            if self._is_semantic_output_style()
            else normalized
        )
        ...
        if isinstance(value, Node):
            ...
            if value.type == "!":
                if not value.children:
                    return "!false"
                operand = self._trim_redundant_outer_parentheses(
                    self._expr(value.children[0])
                )
                return f"!({operand})"

            handler = self._expr_handler_cache.get(
                value.type, self._EXPR_HANDLER_MISSING
            )
            ...
            if handler is not None:
                return cast("Callable[[Node], str]", handler)(value)
    ...
    def _format_int_literal(self, value: object) -> str:
        ...
        if parsed is None:
            return str(value)
        if abs(parsed) > 10 and self.int_format == "hex":
    ...
    def _expr_find_property(self, node: Node) -> str:
        ...
        base = self._property_name(node.children[0])
        return self._alias_activation_slot_name(base, Node("get_scope_object", [0]))

    def _expr_find_property_strict(self, node: Node) -> str:
        if not node.children:
            return "/* find_property_strict */"
        base = self._property_name(node.children[0])
        return self._alias_activation_slot_name(base, Node("get_scope_object", [0]))
    ...
    def _expr_coerce_u(self, node: Node) -> str:
        operand = self._expr(node.children[-1]) if node.children else "0"
        return f"uint({operand})"

    def _expr_coerce_i(self, node: Node) -> str:
        operand = self._expr(node.children[-1]) if node.children else "0"
        return f"int({operand})"

    def _expr_coerce_d(self, node: Node) -> str:
        operand = self._expr(node.children[-1]) if node.children else "0"
        return f"Number({operand})"

    def _expr_coerce_s(self, node: Node) -> str:
        operand = self._expr(node.children[-1]) if node.children else "undefined"
        return f"String({operand})"
    ...
    def _expr_post_increment_property(self, node: Node) -> str:
        if len(node.children) < 2:
            return "property++"
        subject = node.children[0]
        name = node.children[1]
        return f"{self._expr(subject)}.{self._property_name(name)}++"

    def _expr_pre_increment_property(self, node: Node) -> str:
        if len(node.children) < 2:
            return "++property"
        subject = node.children[0]
        name = node.children[1]
        if subject is not None:
            subject_expr = self._expr(subject)
            if subject_expr:
                return f"++{subject_expr}.{self._property_name(name)}"
        return f"++{self._property_name(name)}"

    def _expr_post_decrement_property(self, node: Node) -> str:
        if len(node.children) < 2:
            return "property--"
        subject = node.children[0]
        name = node.children[1]
    ...
    def _expr_pre_decrement_property(self, node: Node) -> str:
        if len(node.children) < 2:
            return "--property"
        subject = node.children[0]
        name = node.children[1]
    ...
    def _expr_new_function(self, node: Node) -> str:
        ...
        numeric_method_index = _index_value(method_index)
        if numeric_method_index is None:
            return f"function /* method_{method_index} */() {{}}"

        context = self.method_context
        if context is None:
            return f"function /* method_{numeric_method_index} */() {{}}"
        if self._is_semantic_style() and not self._activation_slot_alias_map:
            return f"function /* method_{numeric_method_index} */() {{}}"
        method_name = str(getattr(context, "method_name", "") or "")
        if method_name and not method_name.endswith("ErrorHandler"):
            return f"function /* method_{numeric_method_index} */() {{}}"
        return f"function /* method_{numeric_method_index} */() {{}}"
    ...
    def _expr_delete(self, node: Node) -> str:
        if len(node.children) >= 2:
            target = self._property_target(node.children)
            return f"delete {target}"
        return "delete /* malformed */"
    ...
    def _binary(self, node: Node, op: str) -> str:
        ...
        left = self._binary_child_expr(
            node.children[0], parent_op=op, parent_prec=parent_prec, is_right=False
        )
        right = self._binary_child_expr(
            node.children[1], parent_op=op, parent_prec=parent_prec, is_right=True
        )
        return f"{left} {op} {right}"

    def _binary_child_expr(
        self, value: object, *, parent_op: str, parent_prec: int, is_right: bool
    ) -> str:
        ...
        if child_prec < parent_prec:
            ...
            if not (
                parent_op in self._ASSOCIATIVE_BINARY_OPS and child_op == parent_op
            ):
                needs_parentheses = True
    ...
    def _call_property_like(self, node: Node, super_call: bool = False) -> str:
        ...
        if (
            not super_call
            and self._is_semantic_output_style()
            and self._is_redundant_member_access(subject, name)
        ):
    ...
    def _emit_switch(self, node: Node, level: int) -> str:
        ...
        prepared_cond, preamble, sections = self._prepare_switch_layout(
            cond_node, body.children
        )
    ...
    def _prepare_switch_layout(
        ...
        rewritten_cond, sections = self._rewrite_switch_selector_chain(
            cond_node, sections
        )
        sections, has_duplicate_case_bodies = self._collapse_duplicate_switch_sections(
            sections
        )
    ...
    def _emit_switch_sections(
        ...
        for item in preamble:
            if isinstance(item, Node):
                ...
                lines.append(
                    f"{self._pad(level)}/* unsupported literal stmt: {item!r} */"
                )
        ...
        for section in sections:
            ...
            for item in section.body:
                if isinstance(item, Node):
                    ...
                    lines.append(
                        f"{self._pad(level + 1)}/* unsupported literal stmt: {item!r} */"
                    )
    ...
    def _build_switch_sections(
        self, children: list[object]
    ) -> tuple[list[object], list[SwitchSection]]:
        preamble: list[object] = []
        sections: list[SwitchSection] = []
    ...
    def _move_default_to_end_if_safe(
        self, sections: list[SwitchSection]
    ) -> list[SwitchSection]:
        default_idx = -1
        for idx, section in enumerate(sections):
    ...
    def _should_insert_switch_break(
        ...
        if aggressive_group_breaks:
            if self._switch_section_signature(
                section.body
            ) != self._switch_section_signature(sections[next_non_empty].body):
                return True
        return False

    @staticmethod
    def _next_non_empty_switch_section(
        index: int, sections: list[SwitchSection]
    ) -> int | None:
    ...
    def _rewrite_switch_selector_chain(
        ...
        for idx, section in enumerate(rewritten):
            ...
            if mapped_key is None:
                continue
            rewritten_key = self._normalize_switch_case_key(
                self._clone_object(mapped_key)
            )
        ...
        if synthesized_default_idx is not None and explicit_default_indices:
            ...
            rewritten = [
                section
                for idx, section in enumerate(rewritten)
                if idx not in explicit_defaults
            ]
    ...
    def _decode_ternary_switch_selector(
        ...
        while (
            isinstance(cursor, Node)
            and cursor.type in {"ternary", "ternary_if", "ternary_if_boolean"}
            and len(cursor.children) >= 3
        ):
    ...
    def _normalize_switch_case_key(self, value: object) -> object:
        ...
        context_map = (
            self.method_context.avm2_constant_value_map
    ...
    def _is_safe_switch_selector_key(self, value: object) -> bool:
        ...
        if value.type in allowed_leaf:
            return all(
                self._is_safe_switch_selector_key(child) for child in value.children
            )
        if value.type == "get_property":
            ...
            if isinstance(name, Node):
                ...
                return name.type in {
                    ...
                    "integer",
                    "unsigned",
                } and self._is_safe_switch_selector_key(name)
    ...
    def _int_like_value(value: object) -> int | None:
        ...
        if isinstance(value, Node):
            if (
                value.type in {"integer", "unsigned"}
                and value.children
                and isinstance(value.children[0], int)
            ):
    ...
    def _normalized_switch_children(self, children: list[object]) -> list[object]:
        ...
        def _label_sort_key(section: tuple[Node, list[object]]) -> tuple[int, int, str]:
            ...
            if label.type == "case" and label.children:
                ...
                if (
                    isinstance(case_value, Node)
                    and case_value.type in {"integer", "unsigned"}
                    and case_value.children
                ):
    ...
    def _can_sort_switch_sections(
        self, sections: list[tuple[Node, list[object]]]
    ) -> bool:
    ...
    *,
    ...
    if not skip_fast_path:
        fast_text = _try_fast_emit_method_text(
            body, context, int_format=normalized_int_format
        )
...
def _collect_script_method_entries(
    abc: ABCFile, script_index: int
) -> tuple[str, list[_ClassMethodEntry]]:
...
def _decompile_abc_classes_layout_blocks(
    ...
    *,
    ...
    for class_index in range(class_count):
        package_parts, class_name, entries, _ = _collect_class_method_entries(
            abc, class_index
        )
        instance = (
            abc.instances[class_index] if class_index < len(abc.instances) else None
        )
        cls = abc.classes[class_index] if class_index < len(abc.classes) else None
        instance_traits_list = (
        ...
        is_interface = (
            getattr(instance, "is_interface", False) if instance is not None else False
        )
        class_signature = _build_class_signature(
            class_name, instance, abc, is_interface=is_interface
        )
        lines = [f"{class_signature} {{"]
        class_imports: set[str] = set()

        # Build pool index for type lookups (reuse from ImportDiscoveryPass cache)
        pool_index: dict[str, tuple[str, ...]] = {}
        try:
            pool_id = id(abc.constant_pool)
            if pool_id not in _pool_auto_import_cache:
                # Build index if not cached
                from oven.avm2.transform.semantic_passes import ImportDiscoveryPass

                _pool_auto_import_cache[pool_id] = (
                    ImportDiscoveryPass._build_pool_auto_import_index(abc.constant_pool)
                )
            pool_index = _pool_auto_import_cache[pool_id]
        except:
            pass

        # Add class-level type imports (extends and implements)
        if instance is not None:
            # Add extends class
            super_name = getattr(instance, "super_name", None)
            if super_name:
                if hasattr(super_name, "resolve"):
                    ...
                    raw = super_name
                super_token = _short_multiname(raw).strip()
                if super_token and super_token != "*" and super_token != "Object":
                    class_imports.add(super_token)
            ...
            if isinstance(interfaces, list):
                for iface in interfaces:
                    if hasattr(iface, "resolve"):
                        raw = iface.resolve(abc.constant_pool, "multiname")
                    else:
                        raw = iface
                    token = _short_multiname(raw).strip()
                    if token.startswith("[NsSet]::"):
                        token = token[10:]
                    if token and token != "*":
                        # Try to get FQCN from pool index
                        if token in pool_index:
                            candidates = pool_index[token]
                            if candidates:
                                class_imports.add(candidates[0])
                        else:
                            class_imports.add(token)

        current_fqcn = (
            ".".join((*package_parts, class_name)) if package_parts else class_name
        )

        constructor_field_initializers: dict[str, str] = {}
        static_field_initializers: dict[str, str] = {}
        rendered_methods: list[
            tuple[_ClassMethodEntry, MethodBody, MethodInfo, str, str]
        ] = []
        ...
        for entry in entries:
            ...
            try:
                ...
                if entry.method_name != "__static_init__":
                    fast_text = _try_fast_emit_method_text(
                        body, context, int_format=int_format
                    )
                if fast_text is not None:
                    ...
                    nf, context = _build_method_ir(
                        ...
                        style=style,
                        abc=abc,
                        class_traits=class_traits_list,
                        owner_map=owner_map,
                        method_context=context,
                        namespace_cleanup=False,
                    ...
                    if context is not None:
                        class_imports.update(
                            getattr(context, "discovered_imports", set())
                        )
                    ...
                    if (
                        entry.method_name == "__static_init__"
                        and extracted
                        and _is_effectively_empty_method_nf(nf)
                    ):
                        method_text = ""
                    else:
                        method_text = (
                    ...
                    if entry.method_name == "__static_init__":
                        for name, value in extracted.items():
                            if isinstance(name, str) and isinstance(value, str):
                                static_field_initializers[name] = value
                    elif entry.is_constructor:
                        for name, value in extracted.items():
                            if (
                                isinstance(name, str)
                                and isinstance(value, str)
                                and name not in constructor_field_initializers
                            ):
                                constructor_field_initializers[name] = value
                ...
                try:
                    method_text = error_placeholder.format(
                        method_index=entry.method_index,
                        owner_name=class_name,
                        owner_kind="class",
                        error_type=exc.__class__.__name__,
                    ...
                    method_text = (
                        f"/* decompile_error: {exc.__class__.__name__}: {exc} */"
                    )

            if entry.method_name == "__static_init__" and extracted and not method_text:
        ...
        member_lines, _declared_members = _build_class_member_lines(
            ...
            constructor_initializers=constructor_field_initializers,
            static_field_initializers=static_field_initializers,
            int_format=int_format,
        )
        if member_lines:
        ...
        if not rendered_methods:
            ...
            lines.append("}")
            class_source = _wrap_package_block_with_imports(
                package_parts,
                "\n".join(lines),
                tuple(sorted(class_imports)),
            )
            blocks.append(
                _ClassLayoutBlock(
                    class_name=class_name,
        ...
        for index, (entry, body, method_info, signature, method_text) in enumerate(
            rendered_methods
        ):
            if insert_debug_comments:
                lines.append(
    ...
    # Script bodies are intentionally not emitted as synthetic script_<index>.as files
    # when class blocks already exist. For script-only ABCs, keep script methods
    # uncovered so they can still surface via orphan aggregation output.
    if blocks:
        for script_index in range(len(abc.scripts)):
            _, entries = _collect_script_method_entries(abc, script_index)
            for entry in entries:
                covered_methods.add(entry.method_index)

    orphan_bodies = [
        body for body in abc.method_bodies if body.method not in covered_methods
    ]
    if orphan_bodies:
...
def _prepare_output_root(output_root: Path, *, clean_output: bool) -> None:
    ...
    if clean_output:
        ...
        if resolved_output.parent == resolved_output:
            raise ValueError(
                "refusing to clean output_dir that resolves to filesystem root"
            )
...
def _decompile_abc_parsed_to_files(
    ...
    *,
    ...
    normalized_int_format = _normalize_int_format(int_format)
    resolved_owner_map = (
        owner_map if owner_map is not None else _build_method_owner_map(abc)
    )
    blocks = _decompile_abc_classes_layout_blocks(
        abc,
        style=style,
        int_format=normalized_int_format,
        owner_map=resolved_owner_map,
        inline_vars=inline_vars,
        insert_debug_comments=insert_debug_comments,
        debug_include_offset=debug_include_offset,
        debug_include_opcode=debug_include_opcode,
    ...
    for block in blocks:
        output_path = _materialize_output_path(
            output_root, block.package_parts, block.class_name, used_keys
        )
...
def decompile_abc(
    ...
    *,
    ...
    normalized_int_format = _normalize_int_format(int_format)
    return _decompile_abc_cached(
        bytes(abc_data),
        method_idx,
        style,
        layout,
        normalized_int_format,
        bool(inline_vars),
    )
