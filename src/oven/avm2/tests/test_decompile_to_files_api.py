...
def test_decompile_abc_to_files_from_bytes_cleans_output_directory(
    tmp_path: Path,
) -> None:
    output_dir = tmp_path / "out"
    ...
    written = decompile_abc_to_files(
        _fixture_bytes("Test.abc"),
        output_dir,
        style="semantic",
        int_format="dec",
        clean_output=True,
...
def test_decompile_to_files_replaces_file_output_target_when_clean_output_true(
    tmp_path: Path,
) -> None:
...
def test_decompile_to_files_rejects_file_output_target_when_clean_output_false(
    tmp_path: Path,
) -> None:
    output_file = tmp_path / "out"
    output_file.write_text("stale", encoding="utf-8")
