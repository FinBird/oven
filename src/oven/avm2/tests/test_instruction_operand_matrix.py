...
def _pool_for_operand_matrix() -> ConstantPool:
    ...
    multinames = [Multiname(MultinameKind.QNAME, {"namespace": Index(1), "name": Index(1)})]
    return ConstantPool(ints=[], uints=[], doubles=[], strings=strings, namespaces=namespaces, namespace_sets=[], multinames=multinames)
...
def test_parse_instructions_for_less_used_opcodes_parses_operands() -> None:
    ...
    code = bytes([
        Opcode.NewFunction.value, 1,
        Opcode.NewClass.value, 2,
        Opcode.CallStatic.value, 3, 4,
        Opcode.CallMethod.value, 5, 6,
        Opcode.ConstructProp.value, 1, 2,
        Opcode.HasNext2.value, 7, 8,
        Opcode.ApplyType.value, 2,
        Opcode.NewCatch.value, 3,
        Opcode.Debug.value, 1, 2, 9, 10,
        Opcode.Dxns.value, 2,
        Opcode.DebugFile.value, 2,
        Opcode.PushNamespace.value, 1,
        Opcode.BkptLine.value, 11,
        Opcode.GetOuterScope.value, 12,
        Opcode.ReturnVoid.value,
    ])
    instructions = reader.parse_instructions(code, pool)
    opcodes = [inst.opcode for inst in instructions]
    assert opcodes == [
        Opcode.NewFunction, Opcode.NewClass, Opcode.CallStatic, Opcode.CallMethod,
        Opcode.ConstructProp, Opcode.HasNext2, Opcode.ApplyType, Opcode.NewCatch,
        Opcode.Debug, Opcode.Dxns, Opcode.DebugFile, Opcode.PushNamespace,
        Opcode.BkptLine, Opcode.GetOuterScope, Opcode.ReturnVoid,
    ]
...
def test_parse_instructions_pushbyte_is_signed_i8() -> None:
    reader = ABCReader(b"")
    code = bytes([Opcode.PushByte.value, 255, Opcode.PushByte.value, 127, Opcode.ReturnVoid.value])
    instructions = reader.parse_instructions(code, None)
    assert [inst.opcode for inst in instructions] == [Opcode.PushByte, Opcode.PushByte, Opcode.ReturnVoid]
    assert instructions[0].operands == [-1]
...
def test_parse_instructions_for_plain_u30_constructor_and_call_opcodes() -> None:
    ...
    code = bytes([
        Opcode.Call.value, 3,
        Opcode.Construct.value, 2,
        Opcode.ConstructSuper.value, 4,
        Opcode.NewClass.value, 5,
        Opcode.NewFunction.value, 6,
        Opcode.ReturnVoid.value,
    ])
    instructions = reader.parse_instructions(code, None)
    assert [inst.opcode for inst in instructions] == [
        Opcode.Call, Opcode.Construct, Opcode.ConstructSuper,
        Opcode.NewClass, Opcode.NewFunction, Opcode.ReturnVoid,
    ]
...
def test_parse_instructions_branch_operand_shapes_for_jump_and_lookupswitch() -> None:
    reader = ABCReader(b"")
    code = bytes([
        Opcode.Jump.value, 254, 255, 255,
        Opcode.LookupSwitch.value, 5, 0, 0, 1, 2, 0, 0, 255, 255, 255,
        Opcode.ReturnVoid.value,
    ])
    instructions = reader.parse_instructions(code, None)
