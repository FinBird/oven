...
def test_method_body_at_returns_body_for_valid_method_index() -> None:
    abc = ABCFile(
        minor_version=16,
        major_version=46,
        constant_pool=_empty_pool(),
        methods=[_method("m0"), _method("m1"), _method("m2")],
        metadata=[],
        instances=[],
        classes=[],
        scripts=[],
        method_bodies=[_body(0), _body(2)],
    )
...
def test_method_body_at_returns_none_for_missing_method_index() -> None:
    abc = ABCFile(
        ...
        major_version=46,
        constant_pool=_empty_pool(),
        methods=[_method("m0"), _method("m1")],
        metadata=[],
        instances=[],
        classes=[],
        scripts=[],
        method_bodies=[_body(0)],
    )
...
def test_abcfile_decompile_forwards_inline_vars(
...
def test_abcfile_decompile_to_files_forwards_inline_vars(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    abc = ABCFile(
        minor_version=16,
    ...
    monkeypatch.setattr(
        decompiler, "_decompile_abc_parsed_to_files", _fake_decompile_to_files
    )
    written = abc.decompile_to_files(
        "out", style="semantic", int_format="hex", clean_output=False, inline_vars=True
    )
...
def test_fix_names_sanitizes_invalid_identifier_parts() -> None:
    pool = ConstantPool(
        ...
        uints=[],
        doubles=[],
        strings=["123foo.-bar.baz!", "9@bad-name"],
...
def test_fix_names_keeps_http_namespace_unchanged() -> None:
    pool = ConstantPool(
        ...
        uints=[],
        doubles=[],
        strings=["http://example.com/ns.path"],
        namespaces=[NamespaceInfo(NamespaceKind.NAMESPACE, 1)],
        namespace_sets=[],
        multinames=[],
    )
...
def test_fix_names_renames_keywords_and_deduplicates_with_i_suffix() -> None:
    pool = ConstantPool(
        ...
        strings=["for", "for"],
        namespaces=[
            NamespaceInfo(NamespaceKind.NAMESPACE, 1),
            NamespaceInfo(NamespaceKind.NAMESPACE, 2),
        ],
        namespace_sets=[],
        multinames=[],
    )
