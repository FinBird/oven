...
@pytest.fixture
def build_single_method_abc() -> Callable[[bytes, int], bytes]:
    def _build(code: bytes, max_stack: int = 1, num_locals: int = 0) -> bytes:
        blob = bytearray()
...
def build_invalid_lookupswitch_abc(
    ...
    build_single_method_abc: Callable[[bytes, int], bytes],
) -> Callable[[], bytes]:
    def _build() -> bytes:
        code = bytes(
            [
        ...
        return build_single_method_abc(code, 1)
