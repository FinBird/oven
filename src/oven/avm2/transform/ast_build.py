...
class ASTBuild(Transform):
    ...
    _SHORTCUT_START = frozenset({Opcode.ConvertB, Opcode.CoerceB, Opcode.IfTrue, Opcode.IfFalse})
    ...
    def transform(self, code: Sequence[Instruction], body: Any) -> tuple[Node, Any, list[Any]]:
    ...
    def _dispatch(self, index: int, instruction: Instruction) -> None:
        ...
        match opcode:
            ...
            case Opcode.FindPropStrict | Opcode.FindProperty:
                ...
                node_type = "find_property_strict" if opcode == Opcode.FindPropStrict else "find_property"
                self._produce(Node(node_type, [target], self._label(instruction.offset)))
                return

            case Opcode.GetGlobalScope:
                self._produce(Node("get_global_scope", [], self._label(instruction.offset)))
                return

            case Opcode.GetScopeObject:
                scope_index = int(instruction.operands[0]) if instruction.operands else 0
                self._produce(Node("get_scope_object", [scope_index], self._label(instruction.offset)))
                return

            case Opcode.GetLex:
                target = instruction.operands[0] if instruction.operands else None
                self._produce(Node("get_lex", [target], self._label(instruction.offset)))
                return
            ...
            case Opcode.GetSuper:
                ...
                self._produce(
                    Node("get_super", [subject, name, *runtime_name_parts], self._label(instruction.offset))
                )
                return
            ...
            case Opcode.SetSuper:
                ...
                self._flush_conditionals()
                self._emit(
                    Node("set_super", [subject, name, *runtime_name_parts, value], self._label(instruction.offset))
                )
                return
            ...
            case Opcode.GetProperty:
                ...
                runtime_name_parts = values[1:]
                self._produce(
                    Node("get_property", [subject, name, *runtime_name_parts], self._label(instruction.offset))
                )
                return

            case Opcode.GetSlot:
                scope, = self._consume(1, opcode)
                slot_index = int(instruction.operands[0]) if instruction.operands else 0
                self._produce(Node("get_slot", [slot_index, scope], self._label(instruction.offset)))
                return
            ...
            case Opcode.SetProperty | Opcode.InitProperty:
                ...
                runtime_name_parts = values[1 : 1 + runtime_arity]
                value = values[-1]
                node_type = "init_property" if opcode == Opcode.InitProperty else "set_property"
                self._flush_conditionals()
                self._emit(
                    Node(node_type, [subject, name, *runtime_name_parts, value], self._label(instruction.offset))
                )
                return
            ...
            case Opcode.SetSlot:
                ...
                slot_index = int(instruction.operands[0]) if instruction.operands else 0
                self._flush_conditionals()
                self._emit(Node("set_slot", [slot_index, scope, value], self._label(instruction.offset)))
                return
            ...
            case Opcode.NewActivation:
                self._produce(Node("new_activation", [], self._label(instruction.offset)))
                return
            ...
            case Opcode.NewFunction:
                method_index = int(instruction.operands[0]) if instruction.operands else 0
                self._produce(Node("new_function", [method_index], self._label(instruction.offset)))
                return

            case Opcode.NextName:
                obj, index_value = self._consume(2, opcode)
                self._produce(Node("next_name", [obj, index_value], self._label(instruction.offset)))
                return

            case Opcode.NextValue:
                obj, index_value = self._consume(2, opcode)
                self._produce(Node("next_value", [obj, index_value], self._label(instruction.offset)))
                return

            case Opcode.HasNext:
                obj, index_value = self._consume(2, opcode)
                self._produce(Node("has_next", [obj, index_value], self._label(instruction.offset)))
                return

            case Opcode.HasNext2:
                obj_reg = int(instruction.operands[0]) if instruction.operands else 0
                index_reg = int(instruction.operands[1]) if len(instruction.operands) > 1 else 0
                self._produce(Node("has_next2", [obj_reg, index_reg], self._label(instruction.offset)))
                return
            ...
            case op if op in self._SIMPLE_CONVERTS:
                ...
                if convert_type == "integer":
                    ...
                    node_type = "convert"
                    children = [convert_type, value]
                self._produce(Node(node_type, children, self._label(instruction.offset)))
                return

            case Opcode.Kill:
                local_index = int(instruction.operands[0]) if instruction.operands else 0
                self._flush_conditionals()
            ...
            case Opcode.Coerce:
                ...
                self._produce(Node("coerce", [type_name, value], self._label(instruction.offset)))
                return

            case Opcode.AsType:
                value, = self._consume(1, opcode)
                type_name = instruction.operands[0] if instruction.operands else None
                self._produce(Node("as_type", [value, type_name], self._label(instruction.offset)))
                return

            case Opcode.AsTypeLate:
                value, type_name = self._consume(2, opcode)
                self._produce(Node("as_type_late", [value, type_name], self._label(instruction.offset)))
                return

            case Opcode.IsType:
                value, = self._consume(1, opcode)
                type_name = instruction.operands[0] if instruction.operands else None
                self._produce(Node("is_type", [value, type_name], self._label(instruction.offset)))
                return

            case Opcode.IsTypeLate:
                value, type_name = self._consume(2, opcode)
                self._produce(Node("is_type_late", [value, type_name], self._label(instruction.offset)))
                return

            case op if op in self._COERCES:
                value, = self._consume(1, opcode)
                self._produce(Node("coerce", [self._COERCES[opcode], value], self._label(instruction.offset)))
                return

            case Opcode.ConvertB | Opcode.CoerceB:
                value, = self._consume(1, opcode)
                self._produce(Node("coerce_b", [value], self._label(instruction.offset)))
                return
            ...
            case op if op in self._BINARY:
                ...
                self._produce(Node(self._BINARY[opcode], [left, right], self._label(instruction.offset)))
                return

            case op if op in self._UNARY:
                value, = self._consume(1, opcode)
                self._produce(Node(self._UNARY[opcode], [value], self._label(instruction.offset)))
                return
        ...
        self._flush_conditionals()
        node = Node(self._opcode_to_ast_type(opcode), list(instruction.operands), self._label(instruction.offset))
        self._emit(node)
    ...
    def _handle_dup(self, instruction: Instruction) -> None:
        ...
        self._flush_conditionals()
        self._emit(Node("set_local", [temp_local, value], self._label(instruction.offset)))
    ...
    def _handle_conditional(self, insn_index: int, instruction: Instruction) -> None:
        ...
        self._produce(Node(cond_type, params, {"label": instruction.offset, "offset": target}))

    def _handle_jump(self, insn_index: int, instruction: Instruction) -> None:
        target = self._branch_target(insn_index, instruction)
        jump_delta = int(instruction.operands[0]) if instruction.operands else 0
    ...
    def _handle_lookup_switch(self, instruction: Instruction) -> None:
        ...
        self._emit_statement("lookup_switch", [default_target, case_targets, expr], instruction.offset)

    def _handle_call_like(self, instruction: Instruction) -> None:
        name = instruction.operands[0] if instruction.operands else None
        argc = int(instruction.operands[1]) if len(instruction.operands) > 1 else 0
        runtime_arity = self._runtime_multiname_arity(name)
        ...
        node = Node(node_type, [subject, name, *runtime_name_parts, *args], self._label(instruction.offset))
        if void_call:
            self._flush_conditionals()
            self._emit(node)
        else:
    ...
    def _handle_call(self, instruction: Instruction) -> None:
        argc = int(instruction.operands[0]) if instruction.operands else 0
        values = self._consume(argc + 2, instruction.opcode)
        subject = values[0]
        this_obj = values[1]
        args = values[2:]
        self._produce(Node("call", [subject, this_obj, *args], self._label(instruction.offset)))
    ...
    def _handle_construct_prop(self, instruction: Instruction) -> None:
        ...
        args = values[1 + runtime_arity :]
        self._produce(
            Node("construct_property", [subject, name, *runtime_name_parts, *args], self._label(instruction.offset))
        )
    ...
    def _handle_construct(self, instruction: Instruction) -> None:
        ...
        self._produce(Node("construct", [target, *args], self._label(instruction.offset)))
    ...
    def _handle_construct_super(self, instruction: Instruction) -> None:
        ...
        self._flush_conditionals()
        self._emit(Node("construct_super", [subject, *args], self._label(instruction.offset)))

    def _handle_new_object(self, instruction: Instruction) -> None:
        count = int(instruction.operands[0]) if instruction.operands else 0
    ...
    def _handle_new_class(self, instruction: Instruction) -> None:
        ...
        class_index = int(instruction.operands[0]) if instruction.operands else 0
        self._produce(Node("new_class", [base, class_index], self._label(instruction.offset)))
    ...
    def _finalize_complex_expr(
        ...
        while targets and targets[-1] == current_offset:
            ...
            if wrap_to:
                ...
                self._produce(Node(str(node_type), [*prefix, expr], self._label(current_offset)))
    ...
    def _handle_shortcut(self, insn_index: int, instruction: Instruction) -> bool:
        ...
        if opcode == Opcode.Jump and instruction.operands and int(instruction.operands[0]) == 0:
            return True
    ...
    def _branch_target(self, insn_index: int, instruction: Instruction) -> int:
        ...
        relative = int(instruction.operands[0])
    ...
    def _next_offset(self, insn_index: int) -> int:
        if insn_index + 1 < len(self._instructions):
            return int(self._instructions[insn_index + 1].offset)
        return int(self._instructions[insn_index].offset)
    ...
    def _literal_node(self, instruction: Instruction) -> Node | None:
        ...
        if opcode in (Opcode.PushByte, Opcode.PushShort):
            value = int(instruction.operands[0]) if instruction.operands else 0
        ...
        if opcode == Opcode.PushUint:
            value = int(instruction.operands[0]) if instruction.operands else 0
            return Node("unsigned", [value], label)

        if opcode == Opcode.PushDouble:
            value = instruction.operands[0] if instruction.operands else 0.0
            return Node("double", [value], label)

        if opcode == Opcode.PushString:
            value = instruction.operands[0] if instruction.operands else ""
            return Node("string", [value], label)
    ...
    def _local_index(self, instruction: Instruction) -> int:
        ...
        raise ValueError(f"opcode does not represent local access: {instruction.opcode.name}")
    ...
    def _consume(self, count: int, opcode: Opcode) -> list[Node]:
        ...
        if len(self._stack) < count:
            if self.options.get("tolerate_stack_underflow"):
                ...
                self._stack.clear()
                return values
            raise ValueError(f"stack underflow for {opcode.name}: need {count}, have {len(self._stack)}")
    ...
    @staticmethod
    def _label(offset: int) -> dict[str, int]:
        return {"label": int(offset)}
    ...
    def _initial_temp_local(body: Any, instructions: Sequence[Instruction]) -> int:
        ...
        for instruction in instructions:
            ...
            if opcode in ASTBuild._LOCAL_GET:
                ...
                max_seen = max(max_seen, int(instruction.operands[0]))
            elif opcode == Opcode.HasNext2:
                if instruction.operands:
                    max_seen = max(max_seen, int(instruction.operands[0]))
                if len(instruction.operands) > 1:
                    max_seen = max(max_seen, int(instruction.operands[1]))
