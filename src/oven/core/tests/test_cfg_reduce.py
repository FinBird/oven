...
class TestCFGReduce:
    ...
    def test_if_else_diamond(self) -> None:
        ...
        cfg = CFG()
        header = cfg.add_node(label="header", insns=[
            Node("jump_if", [True, "then"])
        ])
    ...
    def test_one_way_if(self) -> None:
        ...
        cfg = CFG()
        header = CFGNode(label="header", instructions=[
            Node("jump_if", [True, "then"])
        ])
    ...
    def test_nested_if(self) -> None:
        ...
        cfg = CFG()
        outer_header = CFGNode(label="outer_header", instructions=[
            Node("jump_if", [True, "inner_header"])
        ])
        outer_header.cti = outer_header.instructions[0]
        inner_header = CFGNode(label="inner_header", instructions=[
            Node("jump_if", [True, "inner_then"])
        ])
    ...
    def test_loop_with_break(self) -> None:
        ...
        header.cti = header.instructions[0]
        body = CFGNode(label="body", instructions=[
            Node("jump_if", [False, "break_target"])
        ])
    ...
    def test_loop_with_continue(self) -> None:
        ...
        header.cti = header.instructions[0]
        body = CFGNode(label="body", instructions=[
            Node("jump_if", [False, "continue_target"])
        ])
        body.cti = body.instructions[0]
    ...
    def test_terminal_block_detection(self) -> None:
        ...
        cfg = CFG()
        terminal = CFGNode(label="terminal", instructions=[
            Node("return_void")
        ])
    ...
    def test_condition_extraction(self) -> None:
        ...
        cfg = CFG()
        header = CFGNode(label="header", instructions=[
            Node("jump_if", [True, Node("get_local", [0])])
        ])
        header.cti = header.instructions[0]
