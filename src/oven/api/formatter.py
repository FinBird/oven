...
class AS3Lexer:
    KEYWORDS = {
        "package", "class", "interface", "function", "var", "const", "public", "private",
        "protected", "internal", "static", "return", "if", "else", "for", "while", "do",
        "switch", "case", "new", "this", "import", "extends", "implements", "try", "catch",
        "finally", "throw", "typeof", "is", "as", "use", "namespace", "super", "void",
        "uint", "int", "Number", "Boolean", "String", "Array", "Object", "Class", "XML"
    }
    ...
    KIND_MAP = {
        'IDENTIFIER': TokenType.IDENTIFIER, 'PUNCTUATION': TokenType.PUNCTUATION,
        'NUMBER': TokenType.NUMBER, 'OPERATOR': TokenType.OPERATOR,
        'STRING': TokenType.STRING, 'COMMENT': TokenType.COMMENT,
    ...
    def _scan_e4x(self, text: str, start: int) -> str:
        ...
        if not struct_re.match(text, pos): return text[pos:pos + 1]
        while pos < len(text):
            ...
            if trigger == '{':
                ...
                if not tag_stack: return text[start:m.end()]
                tag_stack.pop()
                pos = m.end()
                if not tag_stack: return text[start:pos]
                ...
                if text[h_end - 1] == '/':
                    tag_stack.pop()
                    if not tag_stack: return text[start:h_end + 1]
                pos = h_end + 1
    ...
    def tokenize(self, code: str) -> List[Token]:
        ...
        while pos < code_len:
            ...
            if char == '/':
                ...
                if last_t and last_t.value in ('=', ':', '(', '[', ',', 'return', 'throw'):
            ...
            if m:
                ...
                if k == 'IDENTIFIER':
                    ...
                    is_imp = ('!' in v or 'license' in v.lower() or 'copyright' in v.lower())
...
class AS3Processor:
    INDENTS = [" " * (i * 4) for i in range(64)]
    OP_SET = {'=', '==', '!=', '&&', '||', '<', '>', '<=', '>=', '+', '-', '*', '/', '+=', '-=', '=>'}

    ASI_STARTERS = {
        "if", "for", "while", "var", "const", "return", "import", "class", "package",
        "throw", "try", "do", "switch", "public", "private", "protected", "internal",
    ...
    def run(self) -> str:
        return self._emit_minified() if self.config.is_minify else self._emit_formatted()
    ...
    def _emit_minified(self) -> str:
        ...
        for t in self.tokens:
            if t.type == T_COMMENT:
                if not t.is_important_comment: continue
                if res and not res[-1].isspace(): res_append(" ")
                res_append(t.value)
    ...
    def _emit_formatted(self) -> str:
        ...
        for curr, nxt in zip(tokens, nxt_iter):
            ...
            if val == '}':
                ...
                if last_val != '\n': res_append('\n')
                res_append(indents[indent] if indent < 64 else " " * (indent * 4))
            ...
            if (last_code_t and last_val != '\n' and not last_val.isspace() and
                    val not in (';', ',', ')', ':', '.', '[')):
                ...
                if p_type == T_KEYWORD:
                    ...
                    # 标识符/数字/关键字 之间的间距
                    add_s = (p_val == ',') or (p_val == ')' and val == '{') or \
                            (p_type in (T_IDENTIFIER, T_NUMBER) and t_type in (T_IDENTIFIER, T_NUMBER, T_KEYWORD))
                if add_s: res_append(' ')
...
if __name__ == "__main__":
    ...
    def profile_workflow(raw_code: str):
        # 定义一个闭包函数，模拟一次完整流程
        def full_task():
